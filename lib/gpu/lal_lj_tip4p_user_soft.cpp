...
#include "lj_tip4p_user_soft_cl.h"
#elif defined(USE_CUDART)
const char *lj_tip4p_user_soft=0;
#else
#include "lj_tip4p_user_soft_cubin.h"
#endif
...
#include <cmath>
namespace LAMMPS_AL {
#define LJTIP4PUserSoftT LJ_TIP4PUserSoft<numtyp, acctyp>

extern Device<PRECISION,ACC_PRECISION> device;

template <class numtyp, class acctyp>
LJTIP4PUserSoftT::LJ_TIP4PUserSoft(): BaseCharge<numtyp,acctyp>(), _allocated(false) {
}

template <class numtyp, class acctyp>
LJTIP4PUserSoftT::~LJ_TIP4PUserSoft() {
  clear();
}

template <class numtyp, class acctyp>
int LJTIP4PUserSoftT::bytes_per_atom(const int max_nbors) const {
  return this->bytes_per_atom_atomic(max_nbors);
}

template <class numtyp, class acctyp>
int LJTIP4PUserSoftT::init(const int ntypes,
    ...
    const int host_mmax, const double host_w0, int map_size, int max_same,
    double **host_lambda, const double host_nlambda,
    const double host_lam_charge) {
  int success;
  success=this->init_atomic(nlocal,nall,max_nbors,maxspecial,cell_size,gpu_split,
                            _screen,lj_tip4p_user_soft,"k_lj_tip4p_user_soft");
  if (success!=0)
    return success;
  k_pair_distrib.set_function(*this->pair_program,"k_lj_tip4p_user_soft_distrib");
  ...
  taylor.alloc(host_taylor_terms,*(this->ucl_device),UCL_READ_ONLY);
  UCL_H_Vec<numtyp> host_taylor_write(host_taylor_terms,*(this->ucl_device), UCL_WRITE_ONLY);
  for (int i=0; i<host_taylor_terms; i++) host_taylor_write[i]=host_taylor[i];
  ucl_copy(taylor,host_taylor_write,host_taylor_terms,false);

  // Soft-core: allocate lam1 array = pow(lambda[i][j], nlambda)
  lam1.alloc(lj_types*lj_types,*(this->ucl_device),UCL_READ_ONLY);
  // Soft-core shifts packed: sc.x = sc_lj, sc.y = sc_c
  sc.alloc(lj_types*lj_types,*(this->ucl_device),UCL_READ_ONLY);

  // Soft-core parameters: p=2 with alpha tuned for water
  // p=2 ensures ∂r_eff/∂λ = 0 at λ=1 (soft-core correction vanishes at endpoints)
  _alpha_lj = (numtyp)1.0;
  _alpha_c = (numtyp)5.0;
  _power_n = (numtyp)2.0;

  {
    UCL_H_Vec<numtyp> host_lam_write(lj_types*lj_types,*(this->ucl_device), UCL_WRITE_ONLY);
    UCL_H_Vec<numtyp4> host_sc_write(lj_types*lj_types,*(this->ucl_device), UCL_WRITE_ONLY);
    for (int i=0; i<lj_types; i++) {
      for (int j=0; j<lj_types; j++) {
        if (i < ntypes && j < ntypes && host_lambda[i] != nullptr) {
          numtyp lam = (numtyp)pow(host_lambda[i][j], host_nlambda);
          host_lam_write[i*lj_types+j] = lam;
          numtyp4 sc_val;
          // sc_lj = alpha_lj * sigma^6 * (1-lambda)^p
          if (host_lj2[i] != nullptr && host_lj2[i][j] > 0.0) {
            numtyp sigma6 = (numtyp)(host_lj1[i][j] / (2.0 * host_lj2[i][j]));
            numtyp oneml = (numtyp)1.0 - lam;
            sc_val.x = _alpha_lj * sigma6 * oneml * oneml;  // p=2
          } else {
            sc_val.x = (numtyp)0.0;
          }
          // sc_c = alpha_c * (1-lambda)^p
          numtyp oneml = (numtyp)1.0 - lam;
          sc_val.y = _alpha_c * oneml * oneml;  // p=2
          sc_val.z = (numtyp)0.0;
          sc_val.w = (numtyp)0.0;
          host_sc_write[i*lj_types+j] = sc_val;
        } else {
          host_lam_write[i*lj_types+j] = (numtyp)1.0;
          host_sc_write[i*lj_types+j].x = (numtyp)0.0;
          host_sc_write[i*lj_types+j].y = (numtyp)0.0;
          host_sc_write[i*lj_types+j].z = (numtyp)0.0;
          host_sc_write[i*lj_types+j].w = (numtyp)0.0;
        }
      }
    }
    ucl_copy(lam1,host_lam_write,lj_types*lj_types,false);
    ucl_copy(sc,host_sc_write,lj_types*lj_types,false);
  }

  _qqrd2e=qqrd2e;
  _taylor_terms=host_taylor_terms;
  _mmax=host_mmax;
  _user_coeff.x=(numtyp)host_b;
  _user_coeff.y=(numtyp)(0.5/(host_sigma*host_sigma));
  _user_coeff.z=(numtyp)(2.0*log(host_b)/sqrt(2.0*3.14159265358979323846*host_sigma*host_sigma));
  _user_coeff.w=(numtyp)host_w0;
  _lam_charge=(numtyp)host_lam_charge;
  cut_coulsq = host_cut_coulsq;
  cut_coulsqplus = host_cut_coulsqplus;

  du_dlam.alloc(1,*(this->ucl_device), UCL_READ_WRITE);
  ...
  this->_max_bytes=lj1.row_bytes()+lj3.row_bytes()+cutsq.row_bytes()+
      taylor.row_bytes()+sp_lj.row_bytes()+lam1.row_bytes()+sc.row_bytes()+
      this->tag.row_bytes()+this->atom_sametag.row_bytes() +
      hneight.row_bytes()+m.row_bytes()+du_dlam.row_bytes()+
...
template <class numtyp, class acctyp>
double LJTIP4PUserSoftT::host_memory_usage() const {
  return this->host_memory_usage_atomic()+sizeof(LJ_TIP4PUserSoft<numtyp,acctyp>);
}
...
// ---------------------------------------------------------------------------
template <class numtyp, class acctyp>
int LJTIP4PUserSoftT::loop(const int eflag, const int vflag) {
  ...
  if(shared_types) {
    ...
    this->k_pair_sel->run(&this->atom->x, &lj1, &lj3, &_lj_types, &sp_lj,
          ...
          &hneight, &m, &TypeO, &TypeH, &alpha,
          &this->atom->q, &cutsq, &_qqrd2e, &taylor,
          &_user_coeff, &_mmax,
          &cut_coulsq, &cut_coulsqplus, &this->ansO,
          &lam1, &sc, &this->du_dlam, &_lam_charge);
  } else {
  ...
  // Read back du_dlam from device (single-element, accumulated via atomicAdd)
  {
    UCL_H_Vec<acctyp> h_dl(1, *(this->ucl_device), UCL_WRITE_ONLY);
    ucl_copy(h_dl, du_dlam, 1, false);
    du_dlam_total = h_dl[0];
  }
...
template <class numtyp, class acctyp>
void LJTIP4PUserSoftT::copy_relations_data(int n, tagint *tag, int *map_array,
...
// ---------------------------------------------------------------------------
template <class numtyp, class acctyp>
void LJTIP4PUserSoftT::compute(const int f_ago, const int inum_full,
...
// ---------------------------------------------------------------------------
template <class numtyp, class acctyp>
int** LJTIP4PUserSoftT::compute(const int ago, const int inum_full,
                            const int nall, double **host_x, int *host_type,
...
template class LJ_TIP4PUserSoft<PRECISION,ACC_PRECISION>;
}

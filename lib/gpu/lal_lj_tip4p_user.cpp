...
#include "lj_tip4p_user_cl.h"
#elif defined(USE_CUDART)
const char *lj_tip4p_user=0;
#else
#include "lj_tip4p_user_cubin.h"
#endif

#include "lal_lj_tip4p_user.h"
#include <cassert>
#include <cmath>
namespace LAMMPS_AL {
#define LJTIP4PUserT LJ_TIP4PUser<numtyp, acctyp>

extern Device<PRECISION,ACC_PRECISION> device;

template <class numtyp, class acctyp>
LJTIP4PUserT::LJ_TIP4PUser(): BaseCharge<numtyp,acctyp>(), _allocated(false) {
}

template <class numtyp, class acctyp>
LJTIP4PUserT::~LJ_TIP4PUser() {
  clear();
}

template <class numtyp, class acctyp>
int LJTIP4PUserT::bytes_per_atom(const int max_nbors) const {
  return this->bytes_per_atom_atomic(max_nbors);
}

template <class numtyp, class acctyp>
int LJTIP4PUserT::init(const int ntypes,
  ...
  success=this->init_atomic(nlocal,nall,max_nbors,maxspecial,cell_size,gpu_split,
                            _screen,lj_tip4p_user,"k_lj_tip4p_user");
  if (success!=0)
    return success;
  k_pair_distrib.set_function(*this->pair_program,"k_lj_tip4p_user_distrib");
  ...
  this->_max_bytes=lj1.row_bytes()+lj3.row_bytes()+cutsq.row_bytes()+
      taylor.row_bytes()+sp_lj.row_bytes() + hneight.row_bytes()+m.row_bytes()+
...
template <class numtyp, class acctyp>
double LJTIP4PUserT::host_memory_usage() const {
  return this->host_memory_usage_atomic()+sizeof(LJ_TIP4PUser<numtyp,acctyp>);
}
...
// ---------------------------------------------------------------------------
template <class numtyp, class acctyp>
int LJTIP4PUserT::loop(const int eflag, const int vflag) {
...
template <class numtyp, class acctyp>
void LJTIP4PUserT::copy_relations_data(int n, tagint *tag, int *map_array,
...
// ---------------------------------------------------------------------------
template <class numtyp, class acctyp>
void LJTIP4PUserT::compute(const int f_ago, const int inum_full,
...
// ---------------------------------------------------------------------------
template <class numtyp, class acctyp>
int** LJTIP4PUserT::compute(const int ago, const int inum_full,
                            const int nall, double **host_x, int *host_type,
...
template class LJ_TIP4PUser<PRECISION,ACC_PRECISION>;
}

// **************************************************************************
//                               lj_tip4p_user_soft.cu
//                             -------------------
//                              V. Nikolskiy (HSE)
//
//  Device code for acceleration of the lj/tip4p/user/soft pair style
...
ucl_inline numtyp user_taylor_poly(const __global numtyp *restrict taylor,
                                   const numtyp rsq) {
  return (((((taylor[5] * rsq + taylor[4]) * rsq + taylor[3]) * rsq +
             taylor[2]) *
            rsq +
            taylor[1]) *
               rsq +
...
ucl_inline numtyp user_short_sum(const numtyp rsq, const numtyp b,
                                 const numtyp inv_2sigma2, const numtyp energy_coef,
                                 const numtyp w0, const int mmax) {
  numtyp sum = energy_coef * w0 * ucl_exp(-rsq * inv_2sigma2);
  numtyp inv_b = ucl_recip(b);
  numtyp inv_b_pow = inv_b;
  numtyp inv_b2_pow = inv_b * inv_b;
  for (int j = 1; j < mmax; j++) {
    sum += energy_coef * inv_b_pow * ucl_exp(-rsq * inv_2sigma2 * inv_b2_pow);
    inv_b_pow *= inv_b;
...
/* ----------------------------------------------------------------------
   ...
   (Identical to non-soft version — no lambda scaling in distrib.)
---------------------------------------------------------------------- */
__kernel void k_lj_tip4p_user_soft_distrib(
    const __global numtyp4 *restrict x_,
...
/* ----------------------------------------------------------------------
   ...
   SOFT-CORE VARIANT: All forces and energies are multiplied by lam1[mtype]
   = pow(lambda[itype][jtype], nlambda).
---------------------------------------------------------------------- */
__kernel void k_lj_tip4p_user_soft(const __global numtyp4 *restrict x_,
    ...
    const numtyp qqrd2e, const __global numtyp *restrict taylor,
    const numtyp4 user_coeff, const int mmax,
    const numtyp cut_coulsq, const numtyp cut_coulsqplus,
    __global acctyp4 *restrict ansO,
    const __global numtyp *restrict lam1,
    const __global numtyp4 *restrict sc,
    __global acctyp *restrict du_dlam,
    const numtyp lam_charge) {
  int tid, ii, offset;
  ...
  const numtyp b = user_coeff.x;
  const numtyp inv_2sigma2 = user_coeff.y;
  const numtyp energy_coef = user_coeff.z;
  const numtyp w0 = user_coeff.w;
  ...
  if (ii<inum) {
    ...
    for ( ; nbor<nbor_end; nbor+=n_stride) {
      ...
      numtyp factor_lj, special_coul;
      factor_lj = sp_lj[sbmask(j)];
      special_coul = sp_lj[sbmask(j)+4];
      j &= NEIGHMASK;
      ...
      numtyp lam = lam1[mtype];

      if (rsq < lj1[mtype].z) { // cut_ljsq
        // Soft-core LJ: denlj = r^6 + ασ^6(1-λ)^p
        numtyp denlj = rsq*rsq*rsq + sc[mtype].x;
        numtyp r6inv_sc = ucl_recip(denlj);
        // forcelj = λ * r^4 * (lj1.x*r6inv_sc^3 - lj1.y*r6inv_sc^2)
        numtyp forcelj = lam * rsq * rsq *
          (lj1[mtype].x * r6inv_sc * r6inv_sc * r6inv_sc -
           lj1[mtype].y * r6inv_sc * r6inv_sc);
        forcelj *= factor_lj;
        ...
        // Compute LJ pair energy (needed for both energy and du_dlam)
        numtyp e = lj3[mtype].x * r6inv_sc * r6inv_sc -
                   lj3[mtype].y * r6inv_sc;
        // ∂U_LJ/∂λ = U_LJ(1) (unscaled by lam)
        du_dlam_local += factor_lj * (e - lj3[mtype].z);
        if (EVFLAG && eflag) {
          energy += factor_lj * (e - lj3[mtype].z) * lam;
        }
        if (EVFLAG && vflag) {
      ...
      if (rsq < cut_coulsqplus) { //cut_coulsqplus
        ...
        if (rsq < cut_coulsq) {
          numtyp rsq_eff = rsq + sc[mtype].y;
          numtyp r2inv = ucl_recip(rsq_eff);
          numtyp r = ucl_rsqrt(r2inv);
          numtyp rinv = r2inv * r;
          numtyp r3inv = rinv * r2inv;
          numtyp prefactor = qqrd2e * qtmp * qj;  // Coulomb scaled by charges, not lam
          numtyp force_coul;
          if (rsq_eff < (numtyp)2.0)
            force_coul = prefactor * (special_coul * r3inv + user_taylor_poly(taylor, rsq_eff));
          else {
            numtyp force_corr = user_short_force_sum(rsq_eff, b, inv_2sigma2, energy_coef, w0, mmax);
          ...
          // Compute Coulomb pair energy for both energy and du_dlam
          numtyp ec_pair = prefactor * (special_coul * rinv - user_short_sum(rsq_eff, b, inv_2sigma2, energy_coef, w0, mmax));
          if (EVFLAG && eflag) {
            e_coul += ec_pair;
          }

          // ∂U_Coul/∂λ = 2λ·U_Coul(1) = 2·ec_pair/λ
          // (ec_pair = λ²·U_Coul(1) from charge-scaled data file)
          du_dlam_local += (numtyp)2.0 * ec_pair / lam_charge;
          if (EVFLAG && vflag) {
            acctyp3 fd;
        ...
        if (non_local_oxy == 1) {
          ...
          if (rsq < cut_coulsq) {
            ...
            numtyp rinv = r2inv * r;
            numtyp r3inv = rinv * r2inv;
            numtyp prefactor = qqrd2e * x1m.w * qj;  // Coulomb scaled by charges, not lam
            numtyp force_coul;
          if (rsq_eff < (numtyp)2.0)
            force_coul = prefactor * (special_coul * r3inv + user_taylor_poly(taylor, rsq_eff));
          else {
            ...
            // Compute ghost-O Coulomb pair energy
            numtyp ec_pair_ghost = prefactor * (special_coul * rinv - user_short_sum(rsq_eff, b, inv_2sigma2, energy_coef, w0, mmax));
            if (EVFLAG && eflag) {
              e_coul += ec_pair_ghost * (acctyp)0.5 * alpha;
            }

            // ∂U/∂λ for ghost-O: 2·ec_pair_ghost/λ * 0.5*alpha
            du_dlam_local += (numtyp)2.0 * ec_pair_ghost / lam_charge * (numtyp)0.5 * alpha;
            if (EVFLAG && vflag) {
              numtyp4 xH1; fetch4(xH1,iH1,pos_tex);
              numtyp4 xH2; fetch4(xH2,iH2,pos_tex);
              numtyp4 xO;  fetch4(xO,iO,pos_tex);
  ...
  if (t_per_atom>1) {
    ...
    // Reduce du_dlam across threads for this atom
    red_acc[0][tid]=du_dlam_local;
    for (unsigned int s=t_per_atom/2; s>0; s>>=1) {
      simdsync();
      if (offset < s) {
    ...
    // Reduce du_dlam across threads for this atom
    for (unsigned int s=t_per_atom/2; s>0; s>>=1) {
      du_dlam_local += shfl_down(du_dlam_local, s, t_per_atom);
    }
#endif
  }
  if(offset == 0 && ii<inum) {
  ...
  if (offset == 0 && ii < inum) atomicAdd(&du_dlam[0], du_dlam_local * (acctyp)0.5);
  store_answers_q(f,energy,e_coul,virial,ii,inum,tid,t_per_atom,offset,eflag,
                  vflag,ans,engv);
}

__kernel void k_lj_tip4p_user_soft_fast(const __global numtyp4 *restrict x_,

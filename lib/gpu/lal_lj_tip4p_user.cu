...
---------------------------------------------------------------------- */
__kernel void k_lj_tip4p_user_distrib(
    const __global numtyp4 *restrict x_,
...
---------------------------------------------------------------------- */
__kernel void k_lj_tip4p_user(const __global numtyp4 *restrict x_,
    ...
    for ( ; nbor<nbor_end; nbor+=n_stride) {
      ...
      if (rsq < cut_coulsqplus) { //cut_coulsqplus
        ...
        if (rsq < cut_coulsq) {
          ...
          if (EVFLAG && eflag) {
            numtyp ec_pair = prefactor * (special_coul * rinv - user_short_sum(rsq, b, inv_2sigma2, energy_coef, w0, mmax));
            e_coul += ec_pair;
          }
          if (EVFLAG && vflag) {
            acctyp3 fd;
        ...
        if (non_local_oxy == 1) {
          ...
          if (rsq < cut_coulsq) {
            ...
            if (EVFLAG && eflag) {
              numtyp ec_pair = prefactor * (special_coul * rinv - user_short_sum(rsq, b, inv_2sigma2, energy_coef, w0, mmax));
              e_coul += ec_pair * (numtyp)0.5 * alpha;
            }
            if (EVFLAG && vflag) {
              numtyp4 xH1; fetch4(xH1,iH1,pos_tex);
              numtyp4 xH2; fetch4(xH2,iH2,pos_tex);
...
__kernel void k_lj_tip4p_user_fast(const __global numtyp4 *restrict x_,
      ...
      if (rsq < cut_coulsqplus) { //cut_coulsqplus
        ...
        if (non_local_oxy == 1) {
          ...
          if (rsq < cut_coulsq) {
            ...
            if (EVFLAG && eflag) {
              numtyp ec_pair = prefactor * (special_coul * rinv - user_short_sum(rsq, b, inv_2sigma2, energy_coef, w0, mmax));
              e_coul += ec_pair * (acctyp)0.5 * alpha;
            }
            if (EVFLAG && vflag) {
              numtyp4 xH1; fetch4(xH1,iH1,pos_tex);
              numtyp4 xH2; fetch4(xH2,iH2,pos_tex);

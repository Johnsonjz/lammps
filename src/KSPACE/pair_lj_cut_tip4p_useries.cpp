...
static inline double G_Sigma1(double Sigma, double r)//     ˹
{
	return exp(-r * r / (2 * Sigma * Sigma)) / sqrt(2 * MY_PI * Sigma * Sigma);
}

static double Compute_W01(double r0, double b)//    W0
{
	double sum = 0.00;
	for (int i = 1; i < 200; i++)
	{
...
PairLJCutTIP4PUser::PairLJCutTIP4PUser(LAMMPS *lmp) :
...
PairLJCutTIP4PUser::~PairLJCutTIP4PUser()
{
...
void PairLJCutTIP4PUser::compute(int eflag, int vflag)
{
...
void PairLJCutTIP4PUser::settings(int narg, char **arg)
{
  ...
  b = utils::numeric(FLERR, arg[5], false, lmp);
	...
	Mmax= utils::numeric(FLERR, arg[7], false, lmp);
...
void PairLJCutTIP4PUser::init_style()
{
...
double PairLJCutTIP4PUser::init_one(int i, int j)
{
...
void PairLJCutTIP4PUser::write_restart_settings(FILE *fp)
{
...
void PairLJCutTIP4PUser::read_restart_settings(FILE *fp)
{
  if (comm->me == 0) {
...
void PairLJCutTIP4PUser::compute_newsite(double *xO, double *xH1,
...
void *PairLJCutTIP4PUser::extract(const char *str, int &dim)
{
  dim = 0;
...
double PairLJCutTIP4PUser::memory_usage()
{

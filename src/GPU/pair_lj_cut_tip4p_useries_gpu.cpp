...
#include "pair_lj_cut_tip4p_useries_gpu.h"
...
int ljtip4p_user_gpu_init(const int ntypes, double **cutsq, double **host_lj1, double **host_lj2,
...
void ljtip4p_user_gpu_clear();
int **ljtip4p_user_gpu_compute_n(const int ago, const int inum, const int nall, double **host_x,
                                 ...
                                 int *periodicity);
void ljtip4p_user_gpu_compute(const int ago, const int inum, const int nall, double **host_x,
                              ...
                              double *prd);
double ljtip4p_user_gpu_bytes();
void ljtip4p_user_copy_molecule_data(int, tagint *, int *, int, int *, int, int);

/* ---------------------------------------------------------------------- */

PairLJCutTIP4PUserGPU::PairLJCutTIP4PUserGPU(LAMMPS *lmp) :
    PairLJCutTIP4PUser(lmp), gpu_mode(GPU_FORCE)
{
  respa_enable = 0;
  reinitflag = 0;
  cpu_time = 0.0;
...
PairLJCutTIP4PUserGPU::~PairLJCutTIP4PUserGPU()
{
  ljtip4p_user_gpu_clear();
}

/* ---------------------------------------------------------------------- */

void PairLJCutTIP4PUserGPU::compute(int eflag, int vflag)
{
  ev_init(eflag, vflag);
  ...
  if (gpu_mode != GPU_FORCE) {
    ...
    inum = atom->nlocal;
    firstneigh = ljtip4p_user_gpu_compute_n(
    ...
    ljtip4p_user_copy_molecule_data(nall, atom->tag, map_array, map_size,
                                    atom->sametag, max_same, ago);
    ljtip4p_user_gpu_compute(ago, inum, nall, atom->x, atom->type, ilist, numneigh,
...
void PairLJCutTIP4PUserGPU::init_style()
{
  cut_respa = nullptr;
  if (atom->tag_enable == 0)
    error->all(FLERR, "Pair style lj/cut/tip4p/user/gpu requires atom IDs");
  if (!force->newton_pair)
    error->all(FLERR, "Pair style lj/cut/tip4p/user/gpu requires newton pair on");
  if (!atom->q_flag)
    error->all(FLERR, "Pair style lj/cut/tip4p/user/gpu requires atom attribute q");
  ...
  if (atom->map_style == Atom::MAP_HASH)
    error->all(FLERR,
               "GPU-accelerated pair style lj/cut/tip4p/user/gpu currently"
               " requires an 'array' style atom map (atom_modify map array)");

  if (ncoultablebits) {
    if (comm->me == 0)
      error->warning(FLERR,
                     "Pair style lj/cut/tip4p/user/gpu ignores Coulomb tables; forcing table 0");
    ncoultablebits = 0;
  }
  ...
  int success = ljtip4p_user_gpu_init(
...
double PairLJCutTIP4PUserGPU::memory_usage()
{
  double bytes = PairLJCutTIP4PUser::memory_usage();
  return bytes + ljtip4p_user_gpu_bytes();
}
